...
const Funcion = {
  async getAll() {
    const { rows } = await pool.query(`SELECT * FROM ${TABLE} ORDER BY fecha, hora_inicio`);
    return rows;
  },
  ...
  async create(data) {
    ...
    const { rows } = await pool.query(
      `INSERT INTO ${TABLE} (pelicula_id, sala_id, fecha, hora_inicio, precio_base) VALUES ($1, $2, $3, $4, $5) RETURNING *`,
      [pelicula_id, sala_id, fecha, hora_inicio, precio_base]
    );
    return rows[0];
  },
  ...
  async update(id, data) {
    ...
    if (data.hora_inicio !== undefined) { fields.push(`hora_inicio = $${idx++}`); values.push(data.hora_inicio); }
  ...
  async findBySala(salaId) {
    const { rows } = await pool.query(
      `SELECT * FROM ${TABLE} WHERE sala_id = $1 ORDER BY fecha, hora_inicio`,
      [salaId]
    );
    return rows;
  },
  ...
  async findByFecha(fecha) {
    const { rows } = await pool.query(
      `SELECT * FROM ${TABLE} WHERE fecha = $1 ORDER BY hora_inicio`,
      [fecha]
    );
    return rows;
  },
  ...
  async findByPelicula(peliculaId) {
    const { rows } = await pool.query(
      `SELECT * FROM ${TABLE} WHERE pelicula_id = $1 ORDER BY fecha, hora_inicio`,
      [peliculaId]
    );
    return rows;
  },
